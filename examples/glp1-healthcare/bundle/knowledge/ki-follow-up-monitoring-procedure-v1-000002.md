---
type: SOP Knowledge Piece
title: Follow-up Monitoring
description: Staff must record dose tolerance and adverse event symptoms during follow-up.
resource: ../sections/follow-up-monitoring-procedure/section-follow-up-monitoring-procedure-002.md
tags:
- knowledge
- records
- proposed
status: draft
generated:
  actor: sopkb/0.1.0
  date: '2026-09-09'
sources:
- id: src-follow-up-monitoring-procedure
  title: follow up monitoring procedure
  resource: ../sources/follow-up-monitoring-procedure.md
sopkb:
  knowledge_item_id: ki-follow-up-monitoring-procedure-v1-000002
  source_id: follow-up-monitoring-procedure
  source_version_id: follow-up-monitoring-procedure:v1
  section_id: section-follow-up-monitoring-procedure-002
  review_status: proposed
  lifecycle_status: active
  confidence: 0.82
  span_status: exact
  evidence: ../evidence/evidence-ki-follow-up-monitoring-procedure-v1-000002.md
  knowledge_relation: ../relations/kr-ki-follow-up-monitoring-procedure-v1-000002.md
  decision_rules:
  - ../rules/rule-ki-follow-up-monitoring-procedure-v1-000002-records.md
  structured_statement:
    subject: Follow-up Monitoring
    predicate: records
    object: Staff must record dose tolerance and adverse event symptoms during follow-up.
---
# Follow-up Monitoring

## Structured Statement

| Field | Value |
| --- | --- |
| Subject | [Follow-up Monitoring](../concepts/concept-follow-up-monitoring.md) |
| Predicate | `records` |
| Object | Staff must record dose tolerance and adverse event symptoms during follow-up. |

## Evidence

- [evidence-ki-follow-up-monitoring-procedure-v1-000002](../evidence/evidence-ki-follow-up-monitoring-procedure-v1-000002.md)

## Relations

- [kr-ki-follow-up-monitoring-procedure-v1-000002](../relations/kr-ki-follow-up-monitoring-procedure-v1-000002.md)

## Decision Rules

- [Follow-up Monitoring](../rules/rule-ki-follow-up-monitoring-procedure-v1-000002-records.md)

## Source Context

Staff must record dose tolerance and adverse event symptoms during follow-up. [^src-follow-up-monitoring-procedure]

[^src-follow-up-monitoring-procedure]: follow up monitoring procedure section `section-follow-up-monitoring-procedure-002`.
